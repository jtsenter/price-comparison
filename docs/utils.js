...
// "Is my shopping getting dearer?" answered from your own prices and your own
// buying, rather than a national figure covering a basket you do not buy.
//
// Chained, not fixed-basket, and that is the whole design. A fixed basket needs
// the same items priced in every month, and this file has 4 such items across
// the last 12 months - the index would be four products wearing a trenchcoat.
// Each month-on-month link instead uses only the items priced in BOTH of its
// two months, and the links multiply together. Every comparison is like for
// like even though the sample grows from 40 items to 266.
//
// Three guards, each earned on this data:
//  - Coverage. A link whose overlap covers under CPI_MIN_COVERAGE of your
//    shopping trips is thrown away. The 2024-12 -> 2025-01 link rests on 10
//    receipts and reports +10% inflation; that is a sample, not a signal.
//  - Contiguity. Only the most recent UNBROKEN run of good links is charted,
//    so a thin patch truncates the history instead of being bridged over.
//    Today that lands on 2026-05, which is where daily two-store scraping
//    started - the index refuses the receipt era on its own, without a
//    hardcoded date, because a receipt price (what you paid, often on special)
//    and a scraped min(WW, Coles) are not the same measurement.
//  - Outliers. A single item moving more than CPI_MAX_MOVE inside one month is
//    a listing change, not inflation, and one of those in a thin month can move
//    the whole index by a percent.
const CPI_MIN_COVERAGE = 0.6;   // share of your trips a link's overlap must carry
const CPI_MAX_MOVE     = 0.6;   // per-item month-on-month move beyond this is a listing change
const CPI_MIN_POINTS   = 3;     // two points is a line, not a trend
const CPI_MOVERS       = 6;     // biggest contributors shown per direction

// One price per month: the median of that month's daily bests. The median, not
// the mean, so a fortnight on special does not drag the month to the special.
function cpiMonthlyPrices(item, exclusions) {
  const byMonth = new Map();
  for (const p of bwsSeries(item, exclusions)) {
    const m = p.date.slice(0, 7);
    if (!byMonth.has(m)) byMonth.set(m, []);
    byMonth.get(m).push(p.price);
  }
  const out = new Map();
  for (const [m, arr] of byMonth) out.set(m, _median(arr));
  return out;
}

function personalCpi(items, opts) {
  opts = opts || {};
  const exclusions  = opts.exclusions  || {};
  const archivedSet = opts.archivedSet || new Set();
  // trip_count is the only weight available and it is the right one: an item
  // bought 25 times should move this 25 times as much as one bought once.
  // Items never actually bought carry no weight and would only add noise.
  //
  // Deliberately NOT filtered by pw_priorities_v1, unlike every other list on
  // the site. That setting lives in localStorage, so honouring it would make
  // this number differ between your phone and your laptop off the same data -
  // and it is not a small difference: the 36 items currently set to 'archive'
  // move the headline from -4.1% to -6.2%. A measurement has to be reproducible,
  // and "which rows do I want to look at" is not a statement about prices.
  // item.archived and archived_items.json ARE honoured: those are repo-level and
  // mean the item is genuinely out of rotation.
  const pool = (items || []).filter(i =>
    !i.archived && !i._isGroup && (i.trip_count || 0) > 0 && !archivedSet.has(i.list_item));
  if (pool.length < 5) return null;

  const M = new Map(pool.map(i => [i.list_item, cpiMonthlyPrices(i, exclusions)]));
  const months = [...new Set(pool.flatMap(i => [...M.get(i.list_item).keys()]))].sort();
  const allTrips = pool.reduce((s, i) => s + i.trip_count, 0);
  const inBoth = (i, a, b) => {
    const m = M.get(i.list_item);
    if (!m.has(a) || !m.has(b)) return false;
    const r = m.get(b) / m.get(a);
    return r >= 1 - CPI_MAX_MOVE && r <= 1 + CPI_MAX_MOVE;
  };

  const links = [];
  for (let k = 1; k < months.length; k++) {
    const a = months[k - 1], b = months[k];
    const both = pool.filter(i => inBoth(i, a, b));
    let wA = 0, wB = 0, trips = 0;
    for (const i of both) {
      wA += i.trip_count * M.get(i.list_item).get(a);
      wB += i.trip_count * M.get(i.list_item).get(b);
      trips += i.trip_count;
    }
    links.push({ a, b, n: both.length, coverage: allTrips ? trips / allTrips : 0,
                 ratio: wA > 0 ? wB / wA : null });
  }

  let run = [];
  for (const l of links) {
    if (l.coverage >= CPI_MIN_COVERAGE && l.ratio) run.push(l); else run = [];
  }
  ...
  // What moved it. Ranked by trips x price change - the same weighting the index
  // itself uses, so the list explains the line rather than telling a second
  // story. The dollar figure shown is per unit, which is the only per-item
  // number that means anything without inventing a shop.
  const movers = pool.filter(i => inBoth(i, base, latest.month)).map(i => {
    const from = M.get(i.list_item).get(base), to = M.get(i.list_item).get(latest.month);
